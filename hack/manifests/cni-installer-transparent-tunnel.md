...
On each node, the DaemonSet's init container extracts two files out of the
...
- The **legacy `azure0` Linux bridge must not be present**. Transparent-tunnel
  needs every pod packet to leave the physical NIC so VFP sees it. A
  bridge in the path silently bypasses NSG for same-node traffic.
...
roll back existing kernel state (ipset, iptables rules, table 101). To
restore a stock CNI on an existing node, drop a different conflist into
`/etc/cni/net.d` and reboot the node, or run `azure-vnet` `DEL` against
each pod to walk the cleanup path before swapping.
